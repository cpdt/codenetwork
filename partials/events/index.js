...
function EventsPartial(param, db) {
...
EventsPartial.prototype._needsUpdate = co.wrap(function*(since) {
    ...
    if (latestUpdate) return latestUpdate.value > since;
    return true;
});

EventsPartial.prototype._generate = co.wrap(function*(partial, p) {
    let fetchCount = (yield this.configCollection.find({
        key: { $eq: 'eventDisplayCount' }
    }).limit(1).next()).value || 3;
    let events = yield this.eventsCollection.find().limit(fetchCount).sort({ priority: 1 }).toArray();
